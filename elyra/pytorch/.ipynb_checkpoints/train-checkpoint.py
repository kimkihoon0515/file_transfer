...
def train(args):
    ...
    class Net(nn.Module): # 모델 클래스 정의 부분
        ...
        def forward(self, x): # 입력층 -> 활성화 함수(ReLU) -> 은닉층 -> 활성화 함수(ReLU) -> 출력층
            ...
            return x5
    train_dataset = datasets.FashionMNIST(root=mount_path,
                            train=True,
                            transform=transforms.ToTensor(),
                            download=False) # 학습 dataset 정의
                            
    test_dataset = datasets.FashionMNIST(root=mount_path,
                            train=False,
                            transform=transforms.ToTensor(), 
...
if __name__ == "__main__":
  ...
  parser.add_argument('--mount_path',type=str,default='/home/jovyan')
